...
async function loadPowerQueryIoDocs(): Promise<Array<DocumentInterface>>{
  const loader = new SitemapLoader("https://powerquery.io/");
  return loader.load();
}
...
async function ingestDocs() {
  ...
  const powerQueryIoDocs = await loadPowerQueryIoDocs();
  console.debug(`Loaded ${powerQueryIoDocs.length} docs from PowerQuery.io`);

  // if (!smithDocs.length || !apiDocs.length || !langchainDocs.length) {
  //   process.exit(1);
  // }

  if (!powerQueryIoDocs.length) {
    process.exit(1);
  }
